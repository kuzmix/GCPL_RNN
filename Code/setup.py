...
class GCPL_exp:
	...
	def abcent_cycles(self):
		...
		if (self.info['max cycle'] <= self.info['min cycle']) or math.isnan(self.info['max cycle']) or math.isnan(
				self.info['min cycle']):
...
class GCPL_dataset(Dataset):
	...
	def __init__(self, database_list, GCPL_class=GCPL_exp):
		...
		self.info['SoH_next'] = self.info.apply(lambda x: x['Qd_next'] / self.pouches_capacity[x['Pouch']] * 100,
												axis=1)
...
class GCPL_dataset_preparator(Dataset):
	def __init__(self, GCPL_dataset, Preparator, savepath=None):
		...
		self.path = savepath
		for cycle in GCPL_dataset:
			result = self.Preparator(cycle)
			if result[0]:
...
class GCPL_Preparator:
	...
	def __init__(self, timedelta_seconds=60, capacity=150, ):
		self.delta = dt.timedelta(seconds=timedelta_seconds)
	...
	def __call__(self, cycle):
		...
		df = df.resample(self.delta).first()
...
def statistics(dataset):
	info_keys = ['Pouch', 'SoH', "Len", 'Experiment', 'Filename', 'Qc', 'Qd', 'Qc_next', 'Qd_next', 'SoH_next', 'Cycle']
	info = pd.DataFrame({key:[] for key in info_keys})
	for i, cycle in enumerate(dataset):
		...
		info.loc[len(info)] = [pouch, soh, length] + [cycle[key] for key in info_keys[3:]]
